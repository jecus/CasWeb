    ...
    public class AircraftProfile : Profile
    {
        public AircraftProfile()
        {
	        CreateMap<AircraftView, Aircraft>()
		        ...
		        .ForMember(dst => dst.ApuUtizationPerFlightinMinutes,
			        src => src.MapFrom(x => x.ApuUtizationPerFlightinMinutes))
		        ...
		        .ForMember(dst => dst.MaxTakeOffCrossWeight,
			        src => src.MapFrom(x => x.MaxTakeOffCrossWeight ?? default(double)))
		        .ForMember(dst => dst.MaxTaxiWeight, src => src.MapFrom(x => x.MaxTaxiWeight ?? default(double)))
		        .ForMember(dst => dst.MaxZeroFuelWeight,
			        src => src.MapFrom(x => x.MaxZeroFuelWeight ?? default(double)))
		        .ForMember(dst => dst.OperationalEmptyWeight,
			        src => src.MapFrom(x => x.OperationalEmptyWeight ?? default(double)))
		        ...
		        .ForMember(dst => dst.BasicEmptyWeightCargoConfig,
			        src => src.MapFrom(x => x.BasicEmptyWeightCargoConfig ?? default(double)))
		        .ForMember(dst => dst.Model,
			        src => src.MapFrom(x => x.Model));
        }
    }
}